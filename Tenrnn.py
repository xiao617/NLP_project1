...
def SimpleRnnLSTM(X_data,Y_data,X_test,Y_test):
	...
	InputDim = 10
	model = Sequential()
	model.add(LSTM(output_dim=512,batch_input_shape=(Batch_size,1,InputDim)))
	...
	model.summary()
	checkpointer = ModelCheckpoint(filepath="./savemodel/nlpclass10_3#1.hdf5", verbose=1, save_best_only=True)
	his = model.fit_generator(DataSequence(X_data,Y_data,Batch_size),steps_per_epoch=1211,epochs=1000,validation_data=DataSequence(X_test,Y_test,Batch_size),validation_steps=634,callbacks=[checkpointer])
	#print(his)
	with open('./outputlog#10@1.txt','w') as filelog:
...
def main():
	filein = open('Fsenttrain10.json','r')
	rawtrain = json.load(filein)
	filet = open('Fsenttest10.json','r')
